...
class Emotion_embedding(nn.Module):
    ...
    def forward(self, style, emotion, memory):
        emo = self.emo_encoder(emotion)
        sty = self.style_encoder(style).expand_as(memory)

        emo = torch.unsqueeze(emo, 1).expand_as(memory)
        #emo = emo.expand_as(memory)
