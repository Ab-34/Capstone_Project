...
def get_emotion_dict(emotion, gst):
    ...
    emotion_vector = torch.load('/content/drive/MyDrive/GST/GST-Tacotron/embeddings/{emotion}.pth'.format(emotion = emotion.lower()))
...
if __name__ == '__main__':
    ...
    while(True):
      ...
      for k in wavs:
          wav = wavs[k]
          write('/content/drive/MyDrive/GST/GST-Tacotron/Demo/weighted/test/weights_{alpha}gst_{beta}emo_{k}_epoch200_sejalstest.wav'.format(alpha = alpha, beta=beta, k=k), hp.sr, wav)
