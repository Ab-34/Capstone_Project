...
class Tacotron(nn.Module):
    ...
    def forward(self, texts, mels, ref_mels,emo_embedding,alpha):#, emotions):
       ...
       # print(style_embed.shape,ref_mels.shape)
        ...
        emo_embedding  = emo_embedding.expand_as(memory)

        memory = memory + alpha*(style_embed) + (1-alpha)*(emo_embedding) #emb.unsqueeze(1).expand_as(memory) + style_embed
