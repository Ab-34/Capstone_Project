...
class SpeechDataset(Dataset):
    ...
    def __init__(self, r=slice(0, None)):
        ...
        fpaths, texts = get_ESD_data(hp.data, r)
...
def get_ESD_data(data_dir, r):

    # wav_paths = {'Angry':[],"Sad":[],"Neutral":[],"Surprise":[],"Happy":[]}
    # texts = {'Angry':[],"Sad":[],"Neutral":[],"Surprise":[],"Happy":[]}
    emotion_list = ["Angry","Sad","Neutral","Surprise","Happy"]
    wav_paths = []
    texts = []

    for file_number in range(11,21):
...
#                 if os.path.isfile(file_path):
#                   if items[2]==emotion:
#                     wav_paths[emotion].append(file_path)
#                     text = items[1]
#                     text = text_normalize(text) + 'E'
#                     text = [hp.char2idx[c] for c in text]
#                     text = torch.Tensor(text).type(torch.LongTensor)
#                     texts[emotion].append(text)
#             f.close()

#     for wav in wav_paths.keys():
#         wav_paths = wav_paths[r]
#         # print(wav_paths[wav][-5:])
#     print(wav_paths,texts)
#     return wav_paths, texts
'''
