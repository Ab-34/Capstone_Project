...
class SpeechDataset(Dataset):
    ...
    def __init__(self,emotion, r=slice(0, None)):
        ...
        self.emotion_array = []
        if emotion is None: 
          fpaths, texts, emotion_array = get_ESD_data(hp.data, r,None)
          self.emotion_array = emotion_array
          print(self.emotion_array)
        else:
          fpaths, texts= get_ESD_data(hp.data, r,emotion)
          print(fpaths[0])
...
def get_ESD_data(data_dir, r,specific_emotion):
    ...
    if specific_emotion is not None:
      for file_number in [15,16,17,18,19]: # male speakers only
        path = os.path.join(data_dir, '00{file1}/00{file1}.txt'.format(file1 = file_number))
        file_dir = os.path.join(data_dir, '00{file1}/{emotion}/train'.format(file1 = file_number,emotion=specific_emotion))
        # print(file_dir)
    ...
    # print(wav_paths[-5:])
      ...
      for file_number in [15,16,17,18,19]:
        path = os.path.join(data_dir, '00{file1}/00{file1}.txt'.format(file1 = file_number))
