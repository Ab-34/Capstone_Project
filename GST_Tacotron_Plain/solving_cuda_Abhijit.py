...
from Hyperparams_Abhijit import Hyperparameters as hp
...
def train(log_dir, dataset_size, alpha, start_epoch=0):
    ...
    #     train_dataset_amused = SpeechDataset('Amused',r=slice(hp.eval_size, None))
    #     train_dataset_angry = SpeechDataset("Angry",r=slice(hp.eval_size, None))
    #     train_dataset_disgusted = SpeechDataset("Disgusted",r=slice(hp.eval_size, None))
    #     train_dataset_neutral = SpeechDataset("Neutral",r=slice(hp.eval_size, None))
    #     train_dataset_sleepy = SpeechDataset("Sleepy",r=slice(hp.eval_size, None))        
    # else:
    #     train_dataset_amused = SpeechDataset('Amused',r=slice(hp.eval_size, hp.eval_size + dataset_size))
    #     train_dataset_angry = SpeechDataset("Angry",r=slice(hp.eval_size, hp.eval_size + dataset_size))
    #     train_dataset_disgusted = SpeechDataset("Disgusted",r=slice(hp.eval_size, hp.eval_size + dataset_size))
    #     train_dataset_neutral = SpeechDataset("Neutral",r=slice(hp.eval_size, hp.eval_size + dataset_size))
    #     train_dataset_sleepy = SpeechDataset("Sleepy",r=slice(hp.eval_size, hp.eval_size + dataset_size))
    

    # train_loader_amused = DataLoader(dataset=train_dataset_amused, batch_size=hp.batch_size, collate_fn=collate_fn, num_workers=4, shuffle=True)
    # train_loader_angry = DataLoader(dataset=train_dataset_angry, batch_size=hp.batch_size, collate_fn=collate_fn, num_workers=4, shuffle=True)
    # train_loader_disgusted = DataLoader(dataset=train_dataset_disgusted, batch_size=hp.batch_size, collate_fn=collate_fn, num_workers=4, shuffle=True)
    ...
    # train_loader_sleepy = DataLoader(dataset=train_dataset_sleepy, batch_size=hp.batch_size, collate_fn=collate_fn, num_workers=4, shuffle=True)
    ...
    # torch.save(train_dataset,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_dataset_with_emotion_labels.pth')
    # torch.save(train_loader,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_loader_with_emotion_labels.pth')
 

    # torch.save(train_loader_amused,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_loader_amused.pth')
    # torch.save(train_loader_angry,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_loader_angry.pth')
    # torch.save(train_loader_disgusted,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_loader_disgusted.pth')
    # torch.save(train_loader_neutral,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_loader_neutral.pth')
    # torch.save(train_loader_sleepy,'/content/drive/MyDrive/GST/GST_Tacotron/train_datasets_female/train_loader_sleepy.pth')

   

    train_loader_amused = torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_loader_amused.pth')
    train_loader_angry =   torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_loader_angry.pth')
    train_loader_disgusted = torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_loader_disgusted.pth')
    train_loader_neutral = torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_loader_neutral.pth')
    train_loader_sleepy = torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_loader_sleepy.pth')

    train_dataset = torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_dataset_with_emotion_labels.pth')
    train_loader = torch.load('/content/drive/MyDrive/GST_emovDB/GST_Tacotron/train_datasets_female/train_loader_with_emotion_labels.pth')
    ...
    emo_dataset_list = [train_loader_amused, train_loader_angry, train_loader_disgusted, train_loader_neutral, train_loader_sleepy]

    for epoch in range(start_epoch + 1,  hp.num_epochs):
        emotion_dict = {}
        print("Gathering Emotion Info...")
        for j, ele in enumerate(emo_list):
          temp = []
        
          for i, batch in enumerate(emo_dataset_list[j]):
              mels = batch['mel'].to(device)
              # print(mels.shape)
              mels_input = mels[:, :-1, :]  # shift
              mels_input = mels_input[:, :, -hp.n_mels:]  # get last frame
              ref_mels = mels[:, 1:, :].to(device)
        
              with torch.no_grad():
                emo = gst(ref_mels).to('cpu')
                temp.append(emo)

          emotion_dict[j] = torch.mean(torch.cat(temp, dim=0),0)
          del temp
        ...
        for i, batch in enumerate(train_loader):
            ...
            for element in emotion_list:
              ...
              temp.append(emotion_dict[element])
            ...
            memory = memory + alpha*style_embed + (1-alpha)*emotion_vector
        ...
        if epoch % hp.save_per_epoch == 0 and epoch != 0:
            ...
            style_embed = gst(ref_mels)  # [N, 256]
            style_embed = style_embed.expand_as(memory)
            emotion_vector = emotion_dict[0].expand_as(memory)
...
if __name__ == '__main__':
    ...
    # train("./log/male_only_1,1", dataset_size, 0.3, start_epoch)
