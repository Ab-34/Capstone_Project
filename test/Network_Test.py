...
device ='cpu' #torch.device(hp.device)
class Tacotron(nn.Module):
    ...
    def forward(self, texts, mels, ref_mels,emo_embedding):#, emotions):
       ...
       # print(style_embed.shape,ref_mels.shape)
        style_embed = style_embed.expand_as(memory)
        #print(type(emotions), type(emotions[0]))
        #emotions = torch.stack(emotions, dim=0).to(device)
        #print(emotions)#, emotions.shape)
        #print(len(emotions),print(type(emotions[0])))
        # emb = self.emotion_emb(torch.tensor(emotions).to(device) )
        #print(emb.shape)
        #exit()

        #
        # emb = self.emotion_linear(emb)
        #print(emb.shape)
       # print(emb.shape)

        memory = memory + (0.7*style_embed) + (0.3*emo_embedding) #emb.unsqueeze(1).expand_as(memory) + style_embed
