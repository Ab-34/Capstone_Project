...
def predict(text,emo,speaker,idx):
    ...
    if text is not None:
      ...
      if emo is not None:
        # To change
        audio_path =  pred(text,emo,speaker)
        ...
        # audio = audio_file.read()
        # st.audio(audio, format='audio/ogg')
        return "mywav_reduced_noise_{}.wav".format(idx)
        ...
        if Keymax is not None:
          ...
          audio_path =  pred(text,Keymax,speaker)
          original_audio_file = open(audio_path,'rb') 
          audio = original_audio_file.read()
...
def plot_wave(y, sr):
    ...
    return plt.gcf()
def predict_helper(texts_list,emotions_list,speaker = '0011'):
  ...
  with placeholder.container():
    ...
    for T,E in zip(texts_list,emotions_list):
      audio_paths.append(predict(T,E,speaker,i))
      i+=1
    end_1 = timer()
...
if selected == "App":
  ...
  def send_speaker(speaker):
    st.session_state.ans[2] = speaker
  def send_emo(emo):
  ...
  def read_text_emo():
    output = ""
    i = 1
    for TEXT,EMOTION in zip(st.session_state.ans[0],st.session_state.ans[1]):
      if EMOTION is not None:
        output+= str(i) + " " + TEXT + " : " + EMOTION + "\n"
        i+=1
      else:
        output+= str(i) + " " + TEXT + "\n"
        i+=1
    return st.session_state.ans[0],st.session_state.ans[1],output
  ...
  with col2:
      TEXTS,EMOTIONS,st.session_state.OUTPUT = read_text_emo()   
      if len(st.session_state.ans[0]):
          ...
          if len(st.session_state.ans[0]) == len(st.session_state.ans[1]) and len(st.session_state.ans[0])!=0:
            submit = col2.button("Generate Audio")
            if submit:
              predict_helper(TEXTS,EMOTIONS,st.session_state.ans[2])
